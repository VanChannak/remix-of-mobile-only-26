...
const VideoPlayer = ({ 
  ...
  useEffect(() => {
    ...
    const handleNativeFullscreenChange = async () => {
      ...
      // Handle iOS native platform fullscreen state changes (status bar, immersive mode)
      if (Capacitor.isNativePlatform()) {
        if (isFS) {
          // Entering fullscreen - lock to landscape, hide status bar, and enter immersive mode
          try {
          ...
          // Exiting fullscreen - return to portrait, show status bar, and exit immersive mode
          try {
  ...
  // Cleanup: ensure portrait mode, status bar, and exit immersive mode on unmount
  useEffect(() => {
    return () => {
      if (Capacitor.isNativePlatform()) {
        exitImmersiveMode().catch(console.error);
